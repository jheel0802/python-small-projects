...
class product_class:
    def __init__(self,root):
        self.root = root
        self.root.geometry("1020x560+240+105")
        ...
        txt_product_name=Entry(self.root,textvariable=self.var_product_Name,font=("Times New Roman",15),bg="white").place(x=210,y=120,width=190,height=30)
        ...
        self.producttable.place(x=410,y=70,width=600,height=480)
    ...
    def update(self):
        ...
        try:
            if self.var_product_id.get()=="":
                ...
                messagebox.showerror("Error","Rate should be greater than 0",parent=self.root)
            elif self.var_product_Name.get().isalpha()==False:               
                messagebox.showerror("Error","Name must contain alphabets only",parent=self.root)
            elif self.var_rate.get().isnumeric()==False:               
                messagebox.showerror("Error","Rate must contain numbers only",parent=self.root)
            elif self.var_product_id.get().isnumeric()==False:
                messagebox.showerror("Error","Product ID must contain numbers only",parent=self.root)
